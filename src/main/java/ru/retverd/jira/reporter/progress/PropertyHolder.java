...
public class PropertyHolder {
	...
	public PropertyHolder(String propertiesFile) throws IOException {
		...
		regularTabMarker = setStringProperty(properties, REGULAR_TAB_MARKER,
				propertiesFile);

		forbiddenTabMarker = setStringProperty(properties,
				FORBIDDEN_TAB_MARKER, propertiesFile);
		...
		issueDescriptionColumn = setIntProperty(properties,
				ISSUE_DESCRIPTION_COLUMN, propertiesFile);

		issueKeyColumn = setIntProperty(properties, ISSUE_KEY_COLUMN,
				propertiesFile);

		issueEstimationColumn = setIntProperty(properties,
				ISSUE_ESTIMATION_COLUMN, propertiesFile);

		issueSpentColumn = setIntProperty(properties, ISSUE_SPENT_COLUMN,
				propertiesFile);

		issueRemainingColumn = setIntProperty(properties,
				ISSUE_REMAINING_COLUMN, propertiesFile);

		issueStatusColumn = setIntProperty(properties, ISSUE_STATUS_COLUMN,
				propertiesFile);

		startProcessingRow = setIntProperty(properties, START_PROCESSING_ROW,
				propertiesFile);

		issueKeySkipValue = setStringProperty(properties, ISSUE_KEY_SKIP_VALUE,
				propertiesFile);

		issueDescriptionFill = stringToBoolean(setStringPropertyWithDefaults(properties,
				ISSUE_DESCRIPTION_FILL, descriptionFillValues, propertiesFile));

		recalculateFormulas = stringToBoolean(setStringPropertyWithDefaults(properties,
				RECALCULATE_FORMULAS, recalculateFormulasValues, propertiesFile));
	}

	private String setStringProperty(Properties props, String propName,
			String fileName) throws IOException {
		if (props.getProperty(propName) == null) {
			throw new IOException("Missing " + propName + " property in file "
					+ fileName);
		}
		return props.getProperty(propName);
	}

	private String setStringPropertyWithDefaults(Properties props,
			String propName, String[] defValues, String fileName)
			throws IOException {
		...
		if (!Arrays.asList(defValues).contains(propertyValue)) {
			...
			throw new IOException(propName + " property in file " + fileName
					+ " contains inacceptable value: " + propertyValue
					+ ". Only values " + expectedValues + " are acceptable.");
		}
		return propertyValue;
	}

	private int setIntProperty(Properties props, String propName,
			String fileName) throws IOException {
		if (props.getProperty(propName) == null) {
			throw new IOException("Missing " + propName + " property in file "
					+ fileName);
		...
		try {
			...
			throw new IOException("Integer value is expected in property "
					+ propName + " in file " + fileName);
		}
	}
